...
class TrainLoop:
    def __init__(
        ...
        *,
        ...
        if self.model.training:
            ...
            self.model.side_encoder.init_gmm_params(self.data,dist_util.dev())
    ...
    def run_loop(self):
        epochs = math.ceil(self.lr_anneal_steps/len(self.data))
        vae_epochs = epochs//1
        # gmm_epochs = epochs - vae_epochs
        for _ in tqdm(range(vae_epochs)):
            if self.step + self.resume_step >= self.lr_anneal_steps:
                break
            for batch,cond, mask in self.data:
        ...
        # # print('Initializing GMM parameters...')
        # # self.model.side_encoder.init_gmm_params(self.data,dist_util.dev())
        # # self.model.side_encoder.gmm_mode = True
        # for _ in tqdm(range(gmm_epochs)):
        #     if self.step + self.resume_step >= self.lr_anneal_steps:
        #         break
        #     for batch, cond, mask in self.data:
        #         self.run_step(batch, cond)
